...
これによってユーザランドとNICのやりとりがそのままDMAで実現でき、パケットのすばやい送受信が可能となります。
...
本当はPWMがうまくいかなかった原因を探ったり、リンクLEDのみならずステータスLEDも光らせたかった。
でも、データシートを読む気力がなかったので終わりです。
脳とストレージが接続され、e1000のデータシートを読んだ記憶が手に入るようになったらやります。

記事後半はpcspkrとMIDIの話が中心でした。
音を鳴らすことができて良かったです（KONAMI感）。
私も高専時代@<fn>{kosen}はFDD@<fn>{moppy}やシグナルタワーでMIDIを奏でたものですが、それを思い出しました。
このときにSkylineアルゴリズムを知っていればなんかよしなに出来たでしょうね。
同時発音数を@<m>{n}まで許容するような拡張も簡単そうですし@<fn>{sushi}。
//footnote[kosen][大胆な高専アピールは女の子の特権]
//footnote[moppy][@<href>{https://github.com/SammyIAm/Moppy2} を利用]
//footnote[sushi][@<emoji>{sushi}]

あと擬似和音にも挑戦したかったですね。
といいつつ実は簡単に挑戦していて、1音を1 [ms]間隔で区切って鳴らすとそれらしい感じになっていました。
ただ、この辺の時間はなんとなくで決めるものではなさそうなので、いい感じでやっておきたかったです。

さてここからが本題ですが、SRG46ってご存知ですか？
