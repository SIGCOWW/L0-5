...
こじらせてしまい、初任給で抱き枕カバーを増やしてしまいました。
...
そこで今回は、@<b>{DPDK} (Data Plane Development Kit)@<fn>{dprk}をDPDKらしからぬ使い方でNICのLEDを制御します。
...
今回はコンピュータに備わるブザーで音楽を奏でること、またそれに連動してNICのLEDを光らせることも試しています。
...
これによって、ユーザランドのアプリケーションであってもNICとのやりとりにDMAを利用でき、NICで送受信されるデータへのアクセスが高速になります。
...
プロトコルスタックについては、そもそもスイッチやルータを作るなら不要@<fn>{arp}で、そうでなくともSeastar@<fn>{seastar}というフレームワークを使えば楽をできるかも知れません。
Seastarは、独自のTCP/IPプロトコルスタックを備えており、この上で動くアプリケーションの開発を支援します。
サンプルを見ると@<tt>{httpd}というアプリケーションまであり、恐ろしいフレームワークです。
//footnote[arp][ARPパケットを処理したり、ICMP Echo Requestに応答したりくらいはします。]
...
そして、実は@<tt>{e1000_led_on()}関数や@<tt>{e1000_led_off()}関数をたとえ間接的にでもユーザランドから呼ぶには@<tt>{ioctl()}で@<tt>{SIOCETHTOOL}を操作する以外なく、ユーザランドからLEDを自由に制御することはできません。
...
よく見ると、4.のときにSignalを配送すればLEDの点灯時間を0から@<tt>{interval} [tick]まで任意に設定できます。
...
本当はPWMがうまくいかなかった原因を探ったり、リンクLEDのみならずステータスLEDも光らせたりしたかったですね。
でも、データシートを読む気力がなかったので終わりです。
脳とストレージが接続され、e1000で動くNICのデータシートを読んだ記憶が手に入るようになったらやります。

後半はpcspkrとMIDIの話が中心でした。
音を鳴らすことができて良かったです（KONAMI感）。
私も高専時代@<fn>{kosen}はFDD@<fn>{moppy}やシグナルタワーでMIDIを奏でたものですが、それを思い出しました。
このときにSkylineを知っていればなんかよしなに出来たでしょうね。
同時発音数を@<m>{n}まで許容するような拡張も簡単そうですし@<fn>{sushi}。
あと擬似和音にも挑戦したかったですね。
と述べつつ実は少しだけ挑戦していて、1音を1 [ms]間隔で区切って鳴らすとそれらしい感じになっていました。
ただ、この間隔はなんとなく決めるものではなさそうなので、いい感じでやっておきたかったです。
//footnote[kosen][大胆な高専アピールは女の子の特権]
//footnote[moppy][@<href>{https://github.com/SammyIAm/Moppy2} を利用していました。]
//footnote[sushi][暑さ・寒さ・徹夜だって平気ですしおすし@<emoji>{sushi}]

さてここからが本題ですが、皆さんはSRG46ってご存知ですか？
