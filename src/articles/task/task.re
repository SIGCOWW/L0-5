...
古くから研究の対象とされてきました．
JSPは，定義が単純かつ分かりやすい一方，厳密解法が判明していない困難な問題でもあるため，
@<b>{皆さん大好き巡回セールスマン問題}と共に，近似解法のベンチマークとしても広く利用されています．

今回は，JSPを含めたスケジューリング問題，およびその解法について説明します．
...
また，最初の仕事の開始時点（@<img>{make1}では機械Aが仕事Aの処理を開始する時点）から，最後の仕事の終了時点（@<img>{make1}では機械Cが仕事Cの処理を終了する時点）までの時間を@<b>{メイクスパン}
と呼びます．
スケジューリング問題は，このメイクスパンを最小にすることが主な目的になります@<fn>{makespan}．
//footnote[makespan][メイクスパンを最小にすること以外を目的とする応用もあります．]
...
//image[make2][@<img>{schedule}におけるガントチャートの別例．@<img>{make1}よりもメイクスパンが短くなっていることに注目されたい．][scale=1.0]

@<img>{make2}を見ると，@<img>{make1}よりもメイクスパンが短くなっていることが分かります．
このように，単純な例でも仕事の処理手順によってメイクスパンが変化することが分かっていただけるかと思います．
...
: フローショップ問題 (FSP; Flow-shop Scheduling Problem)
  仕事を処理する機械の順番が決まっており，すべての仕事で処理順序が同じ問題を指します．@<img>{schedule}を例にとると，すべての仕事は，必ず機械A，機械B，機械Cの順番で処理しなければなりません．
...
JSPにおける仕事の各工程において，複数の機械を選択可能とした拡張をフレキシブル・ジョブショップスケジューリング問題（F-JSP; Flexible JSP）と呼びます．
...
//imagew[john][ジョンソン法による操作の例．まず，最も処理時間が短いJ3-M1対(3)が選択される．M1は前工程機械であるため，J3は最初に配置される．次に，残った仕事の中で最も処理時間が短いJ4-M2対(4)が選択される．M2は後工程機械であるため，J4は最後に配置される．図では省いたが，次に選択されるのはM1-J2対(6)であり，J2はJ3の後に配置される．よって，この例の解は{J3,J2,J1,J4}となる．][scale=1.0]
...
最後に，社会人生活で余裕がなかったとはいえ，炎上執筆にlrks君を巻き込んでしまった罪を告白します．
この罪に関しては，スケジューリング問題の知見を活かし，次回新刊に圧倒的コミットすることで償う予定です．
