...
: オープンショップ問題
  仕事を処理する機械の順番が決まっていない問題を指します．@<img>{schedule}を例にとると，仕事Aを処理するのに，機械C，機械B，機械Aの順番で処理して良いということになります．

: フローショップ問題
  仕事を処理する機械の順番が決まっており，すべての仕事で処理順序が同じ問題を指します．@<img>{schedule}を例にとると，仕事Aを処理するためには，必ず機械A，機械B，機械Cの順番で処理しなければなりません．

: ジョブショップ問題
  仕事を処理する機械の順序が決まっており，各仕事において処理順序が違って良い問題を指します．@<img>{flowjob}に，フローショップ問題とジョブショップ問題の違いを示します．
  フローショップ問題は，ジョブショップ問題の特殊例と考えることができます．

//image[flowjob][フローショップ問題とジョブショップ問題の比較．フローショップ問題ではすべての仕事の処理順序が同じであるのに対し，ジョブショップ問題ではこの制約が存在しない．][scale=1.0]

=== その他の制約
ジョブショップスケジューリングは，最も実用に近いと書きましたが，実際には
...
また機会があれば，ソルバの詳しい説明と，実装によるベンチマークを行う...かもしれません．
内容薄くてすまんな．
