...
== はじめに (真面目なこと)
先日Natureにて「The Rosa genome provides new insights into the domestication of modern roses」
という論文が掲載されました。
...
今回は、集合の類似度の指標としてjaccard係数、Dice係数、Simpson係数、Tanimoto係数を使用します。
各係数の導出方法は以下の通りです。
各式の@<m>{A,B}は任意の集合であり、@<m>{|A|,|B|}は集合@<m>{A,B}の要素数を表します。
...
また、ベクトルの類似度の指標としてはコサイン類似度を使用します。
コサイン類似度の計算式は以下の通りです。
@<m>{|V|}はベクトルの次元数を表します。
...
今回はアミノ酸配列データベースUniProtから、学名で一致するデータをすべてダウンロードします。
バラの場合「Rosa」で検索してヒットした配列をすべて対象とします。
以下に今回集めた生物種と学名、件数を示します。
さらにその花または果物から連想できるデレマスのアイドルも示します。

加えてアイドルのイラストを@<img>{chara-list}に示します。

//image[chara-list][イラスト一覧][scale=0.7]

== ももありよりももかおでした
n-gram (n=2,3,4,5)の集合を対象として、各指標でバラとの類似度を計算しました。
類似度が1に近い順で順位付けした結果を示します。
なお、学名・生物種名は長いのでアイドルの名前で表記します。
...
まぁ、花なんだから類似してるのは容易に想像できますが。
さらに安定して愛梨が高い順位を保っていました。
