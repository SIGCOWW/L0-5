...
実際にバ美肉に成功したおじさん@<fn>{ojisan}は「バ美肉おじさん」と呼ばれています。
本稿では、外見（2Dまたは3Dアバター）だけでなく、声も美少女化したいという欲張りなバ美肉おじさんの願望を叶えるための技術を提案します。
//footnote[ojisan][年齢的に実際におじさんかどうかは不明であることが多いですが、本稿では一貫して「おじさん」と称することにします。]

== バ美肉おじさんの種別
VTuberがここまで流行している背景には「仮想的なアバターによる自己表現の拡張」及び「自己承認欲求の解消」という2つの欲求があると考えられます。
「現実の世界に疲れたおじさんが美少女のアバターの皮を被り、インターネットの世界でチヤホヤされる」というバ美肉行為は、
これら2つの欲求を同時に満たす非常に合理的な手段であると言えます。
本節では、バ美肉おじさんの種別を、代表的なVTuberを例に挙げながら解説します。

=== 外見の美少女化を用いたバ美肉おじさん
バ美肉を実現するためには、何よりもまず外見を美少女化する必要があります。
...
: （1） 音声認識と音声合成の利用
  「生声での発声→音声認識→VOICEROIDなどの音声合成ソフトでの発声」というシンプルな方法で声の美少女化を実現します。
  仕組みがシンプルであるという利点がありますが、
  音声認識・合成に伴う発話遅延や、
...
: （2） ボイスチェンジャーの利用
  ...
  リアルタイムでの声質変換が可能ですが、十分な音質の音声を生成するためのある程度の訓練が必要、と言われています@<fn>{koigoeusing}。
...
: （3） 統計的声質変換技術の利用
  ...
  自分の声を好きな女優や声優の声に変換する技術が実現できます@<fn>{L0-3-vc}。
...
== 統計的声質変換技術の概要
声質変換とは、入力された音声の言語的情報（発話内容）を保持しながら、それ以外の情報（例えば、話者性）を変換する技術です。
...
声質変換モデルのアーキテクチャは、L0 Vol.3と同様のinput-to-output highway networks@<fn>{highway}（@<img>{highway}）を用いました。
ここで、
...
//footnote[r9y9][@<href>{http://r9y9.github.io/blog/2017/10/05/ganvc/}]
...
本稿では、
「変換元話者が裏声で発話することにより、変換先話者が女性であってもそれなりに女性らしい高さの声を出せる」
という仮定に基づき、裏声の音声波形に対して差分スペクトル法を直接適用する手法を提案します。
具体的な手順は非常にシンプルで、
（1）変換元話者の男性が裏声でパラレルコーパスを収録、
（2）変換元の裏声から変換先話者への変換を（メルケプストラム係数のドメインで）行うモデルを学習、
...
「裏声の男性話者から女性話者への変換を行う手法は、それなりにうまく動作する」
と言えそうです。
